  ...
  try {
    ...
    if (!success) {
      ...
      if (sendToNodeRed && nodeRedData) {
        // Add adjust volumes to nodeRedData
        const extractVolumeInMl = (str) => {
          if (!str) return 0;
          const match = str.match(/(\d+(?:\.\d+)?)\s*(ลิตร|l|ml|มล)/i);
          if (match) {
        ...
        nodeRedData.auto_feed_208n_adjust_ml = extractVolumeInMl(
          getVal("txt_adjust_208n"),
        );
        nodeRedData.auto_feed_208t_adjust_ml = extractVolumeInMl(
          getVal("txt_adjust_208t"),
        );
        nodeRedData.auto_feed_208a_adjust_ml = extractVolumeInMl(
          getVal("txt_adjust_208a"),
        );
        nodeRedData.auto_feed_208b_adjust_ml = extractVolumeInMl(
          getVal("txt_adjust_208b"),
        );
        ...
        try {
          ...
          const nodeRedResponse = await fetch(
            "http://192.168.1.129:1880/data",
            {
              method: "POST",
              headers: {
          ...
          if (nodeRedResponse.ok) {
            ...
            console.warn(
              "Failed to send data to Node-RED:",
              nodeRedResponse.status,
            );
          }
        } catch (nodeRedError) {
...
function deleteUploadedImage(imageName) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      const deletedList = deletedInput.value
      ...
      if (uploadedList.children.length === 0) {
        document.getElementById("uploaded-images-container").style.display =
          "none";
      }
  ...
  empnoTimeout = setTimeout(async () => {
    try {
      const response = await fetch(
        `${basePath}/Home/GetEmployeeByEmpno?empno=${encodeURIComponent(empno)}`,
      );
...
function calculateMaxMin() {
  ...
  document.getElementsByName("txt_max_result")[0].value =
    maxR !== -Infinity ? maxR : "";
  document.getElementsByName("txt_min_result")[0].value =
    minR !== Infinity ? minR : "";
  ...
  document.getElementsByName("txt_max_zn")[0].value =
    maxZn !== -Infinity ? maxZn : "";
  document.getElementsByName("txt_min_zn")[0].value =
    minZn !== Infinity ? minZn : "";
  ...
  document.getElementsByName("txt_max_ni")[0].value =
    maxNi !== -Infinity ? maxNi : "";
  document.getElementsByName("txt_min_ni")[0].value =
    minNi !== Infinity ? minNi : "";
...
document
  .getElementById("us-208t")
  .addEventListener("input", updateAutoFeed208T);
...
document.addEventListener("keydown", function (event) {
  ...
  if (
    name.startsWith("txt_result_") ||
    name.startsWith("txt_zn_") ||
    name.startsWith("txt_ni_")
  ) {
    return;
  }
...
document.addEventListener("keydown", function (event) {
  ...
  const currentColumnInputs = Array.from(
    document.querySelectorAll(`input[name^="${prefix}"]`),
  );
  ...
  if (key === "Enter" || key === "ArrowDown") {
    ...
    if (currentColumnIndex < columns.length - 1) {
      ...
      const nextInputs = Array.from(
        document.querySelectorAll(`input[name^="${nextColumn}"]`),
      );
  ...
  if (key === "ArrowUp") {
    ...
    if (rowIndex === 0 && currentColumnIndex > 0) {
      ...
      const prevInputs = Array.from(
        document.querySelectorAll(`input[name^="${prevColumn}"]`),
      );
